...
let handler = async (m, { conn }) => {
  ...
  if (/image/.test(mime)) {
    ...
    try {
      ...
      let kemii = await Func.fetchJson(`https://api.neoxr.eu/api/whatanime?url=${imageUrl}&apikey=${global.neoapi}`)
      let caption = `• *Title:* ${titleRomaji} (${titleNative})
• *Episode:* ${episode}
• *Similarity:* ${(similarity * 100).toFixed(2)}%
• *At:* ${at}`;
      conn.sendMessage(m.chat, {
      video: {url: kemii.data.video},
      gifPlayback: true, 
      caption: caption
      }, {quoted: m})
    } catch (e) {
    ...
    conn.reply(m.chat, 'Send/Reply Images with the caption *.whatanime*', m)
  }
};
