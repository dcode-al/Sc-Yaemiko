...
let handler = async (m, { conn, text, usedPrefix, args, participants }) => {
  ...
  if (e.length) await conn.reply(res, `Ada @${anubot} Owner-ku Di Sini, halo mas Indra, saya gabung di sini karena di ada yang sewa

@${conn.user.jid.split(`@`)[0]} Terimakasih telah mengundang *${m.name}*`, fkonn, {
    mentions: d
     }).then(async () => {
     await delay(5000)
     }).then( async () => {
     await conn.reply(res, `Semoga ada saya group ini jadi rame`, 0)
