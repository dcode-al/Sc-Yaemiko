...
let handler = async (m, { conn, args, usedPrefix, isOwner }) => {
    ...
    try {
        ...
        let _waktutionskh = (300000 - __waktutionskh);
        let waktutionskh = clockString(_waktutionskh);
        if (new Date - global.db.data.users[m.sender].judilast > 300000) {
            global.db.data.users[m.sender].judilast = new Date * 1;
            let randomaku = `${Math.floor(Math.random() * 69)}`.trim();
            let randomkamu = `${Math.floor(Math.random() * 180)}`.trim(); // hehe Biar Susah Menang :v
            let Aku = (randomaku * 1);
