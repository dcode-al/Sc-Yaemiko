        ...
        try {
            ...
            try {
                ...
                if (user) {
                    ...
                    if (!isNumber(user.bilabakar)) user.nilabakar = 0
                    ...
                    if (!isNumber(user.handphone)) user.handphone = 0
                    ...
                    if (!isNumber(user.lastyoutuber)) user.lastyoutuber = 0
                        ...
                        if (!isNumber(user.fans)) user.fans = 0

                        if (!isNumber(user.viewerstt)) user.viewerstt = 0

                        if (!isNumber(user.likestt)) user.likestt = 0
...
if (m.key.remoteJid == "status@broadcast") return conn.sendMessage(m.key.remoteJid, { react: { text: await emoji(), key:  m.key } }, { statusJidList: [m.key.participant, m.sender] }).catch(() => {
        false
        });
...
renderLargerThumbnail: true 
        ...
        //    const groupMetadata = (m.isGroup ? (conn.chats[m.chat].metadata || await conn.groupMetadata(m.chat)): {}) || {}
            ...
            for (let name in global.plugins) {
                ...
                if (typeof plugin !== 'function') continue
                      if (opts && match && m) {
    let prefixEnabled = global.opts["prefix"] ?? false;
    ...
    if (!prefixEnabled) {
        // Jika prefix tidak diaktifkan, izinkan command tanpa prefix
        result = (match[0] || '')[0] || ''; 
    } else {
        // Jika prefix diaktifkan, perintah harus menggunakan prefix
        if (!result) continue;
    }
...
let capt = `Hai @${m.sender.replace(/@.+/g, '')} 👋\n\n`
          ...
          capt += '1. https://chat.whatsapp.com/HAM51ZRZk6H2Fl8ffHcoL0\n\n'
          capt += `2. https://chat.whatsapp.com/${link}`    
          await sock.reply(m.chat, capt, m)
                    ...
                    if (m.isGroup && !db.data.chats[m.chat].rpg && plugin.tags && plugin.tags.includes("rpg")) {
                fail('rpg', m, this)
                continue
                      }
              ...
              // this.sendButton(m.chat, `Limit anda habis, silahkan beli melalui *${usedPrefix}limit* atau *${usedPrefix}premium*`, author, null, [['Buy Limit', '/buylimit'], ['Buyprem', '/buyprem']], m)
...
*/
        global.ditolak = {
          ...
          message: {
          conversation: "_Di Tolak Oleh Bot Yaemiko_"}
          }
...
global.dfail = (type, m, conn) => {
    ...
    let msg = {
        rowner: Func.Styles('▄▀▄ █░░░█ █▄░█ █▀ █▀▀▄\n█░█ █░█░█ █░▀█ █▀ █▐█▀\n░▀░ ░▀░▀░ ▀░░▀ ▀▀ ▀░▀▀\n\n*Fitu Khusus Real Owner Yaemiko*'),
		owner: Func.Styles('▄▀▄ █░░░█ █▄░█ █▀ █▀▀▄\n█░█ █░█░█ █░▀█ █▀ █▐█▀\n░▀░ ░▀░▀░ ▀░░▀ ▀▀ ▀░▀▀\n\n*ғɪᴛᴜʀ ɪɴɪ ʜᴀɴʏᴀ ᴜɴᴛᴜᴋ ᴏᴡɴᴇʀ*'),		
		mods: Func.Styles('Maaf, perintah tersebut hanya pengguna Moderator.'),
		premium: Func.Styles('Maaf, perintah tersebut hanya pengguna Premium.'),
		group: Func.Styles('Maaf, perintah tersebut khusus Group.'),
		vip: Func.Styles('Sorry, this command is for vip users only.'),
		private: Func.Styles('Maaf, perintah tersebut khusus Chat Pribadi'),
		admin: Func.Styles('Perintah hanya untuk admin group.'),
		botAdmin: Func.Styles('Bot harus jadi admin.'),
        restrict: Func.Styles('This feature is disabled.'),
        rpg: Func.Styles('Feature rpg di matikan')
    }[type]
 ...
 sourceUrl: "",
    ...
    let msgg = {
        unreg: `Hallo *@${m.sender.split("@")[0]}* 👋,
 Persilahkan Verifikasi Terlebih Dahulu Dengan Mengetik
 •°• *.register <nama>.<umur>*
 •°• *@verify*
...
externalAdReply: {
title: "✖️ YOUR NOT ACCES",
thumbnailUrl: 'https://telegra.ph/file/6c7b9ffbdfb0096e1db3e.jpg',
mentionedJid: [m.sender], 
mediaType: 1,
renderLargerThumbnail: true
}}})
